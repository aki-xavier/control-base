// adapt.rs — the calibration's own arithmetic. Why assertions that need an absolute value spell it
// out: the rule approaches the null from BELOW, so a signed comparison would be satisfied by a trim
// that never arrives.

/// Why the model is synthetic and linear: with err(g) = e0 - c * g the fixed point e0 / c is known by
/// hand, so the rule can be asserted rather than eyeballed.
...
fn the_integral_rule_comes_to_rest_at_the_null() {
    ...
    // the trim starts at 0.0 (`add` writes it, `learn` returns it), so the 1000 steps below walk 0 up
    // to the null: measured, g ends 3.6e-3 short, which is what the 1e-2 tolerance is sized for
    // (0.996^1000 leaves 1.8% of the walk unrun)
    let mut a = Adapt::new();
...
fn the_limits_stop_a_calibration_walking_off() {
    // a correction larger than anything ever tested must not be reachable: a persistent error drives
    // the trim to its limit and stays there, and the error it still sees reports the ceiling
...
fn an_unknown_parameter_is_inert() {
    // a knob that was never registered gets no correction instead of a
...
fn a_frozen_parameter_still_reports() {
    ...
    // the readout is pinned whole — the numbers it reports stay visible in the suite — and byte for
    // byte whatever the shortest round-tripping rendering of these values gives (rate 0.0 is where a
    // `%.3g`-style rendering and the shortest form agree)
    assert_eq!(
        a.report()[0],
...
/// pins is the arithmetic: the scale is exactly 1 at a zero trim (the behaviour with no learning in
/// it, bit for bit), the trim integrates the error handed in, it is bounded on both sides, and a
/// frozen one stays where it is.
#[test]
fn the_lead_trim_integrates_the_along_track_error_and_is_bounded() {
    ...
    // contract this calibration requires
...
/// The audit refuses nothing (the calibration stays arithmetic); it makes the mistake readable.
#[test]
...
/// THE CONVERGENCE THIS TIMING STRUCTURE EXISTS FOR: an error delivered N samples late
/// must still converge, for a STATED range of N, to the same value it reaches with no delay.
///
/// The model is the file's opening `converge` one, with the delay where it actually arises: the error
/// at sample k is `e0 - c * g`, `g` being the trim DRIVEN N samples earlier. N = 1..=8 brackets the
/// one-step delays a per-step channel is reached through. No output: the numbers are the assertions.
#[test]
fn the_aged_credit_converges_over_a_stated_range_of_delays() {
    ...
    for n in 1..=8usize {
        // the aged channel: each error is the outcome of the trim driven n samples ago, and
        // `learn_at` credits it to exactly that value
        ...
        // the immediate rule on the SAME delayed model: the same errors, credited to the value now
